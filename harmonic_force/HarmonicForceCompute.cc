...
HarmonicForceCompute::HarmonicForceCompute(
    ...
    std::shared_ptr<ParticleGroup> group, py::list position_lattice,
    py::list orientation_lattice, Scalar translational_force_constant,
    Scalar rotational_force_constant)
    : ForceCompute(sysdef),
      m_group(group),
      translational_force_constant(translational_force_constant),
      rotational_force_constant(rotational_force_constant) {
  unsigned int group_size = m_group->getNumMembersGlobal();
  if (group_size == 0) {
    m_exec_conf->msg->error()
        << "Creating a HarmonicForce with an empty group" << std::endl;
  ...
  for (unsigned int i = 0; i < len(position_lattice); i++) {
    ...
    if (len(tmp_position) != 3)
      throw std::runtime_error(
          "Non-3D position given for HarmonicForceCompute");
    c_position_lattice.push_back(make_scalar3(
        py::cast<Scalar>(tmp_position[0]), py::cast<Scalar>(tmp_position[1]),
        py::cast<Scalar>(tmp_position[2])));
  }
  ...
  for (unsigned int i = 0; i < len(orientation_lattice); i++) {
    ...
    if (len(tmp_orientation) != 4)
      throw std::runtime_error(
          "Non-quaternion orientation given for HarmonicForceCompute");
    c_orientation_lattice.push_back(
        make_scalar4(py::cast<Scalar>(tmp_orientation[0]),
  ...
  if (c_position_lattice.size() != group_size) {
    throw std::runtime_error(
        "Positions given for HarmonicForceCompute doesn't match particle "
        "number.");
  }
  if (c_orientation_lattice.size() != group_size) {
    throw std::runtime_error(
        "Orientation given for HarmonicForceCompute doesn't match particle "
        "number.");
  }
  ...
  ArrayHandle<Scalar3> h_position_lattice(m_position_lattice,
                                          access_location::host);
  ArrayHandle<Scalar4> h_orientation_lattice(m_orientation_lattice,
                                             access_location::host);
...
HarmonicForceCompute::~HarmonicForceCompute() {
  m_exec_conf->msg->notice(5)
      << "Destroying HarmonincForceCompute" << std::endl;
}

void HarmonicForceCompute::setForces() {
  // Create array handles
  ArrayHandle<Scalar3> h_position_lattice(
      m_position_lattice, access_location::host, access_mode::read);
  ArrayHandle<Scalar4> h_orientation_lattice(
      m_orientation_lattice, access_location::host, access_mode::read);

  ArrayHandle<Scalar4> h_force(m_force, access_location::host,
                               access_mode::overwrite);
  ArrayHandle<Scalar4> h_torque(m_torque, access_location::host,
                                access_mode::overwrite);

  ArrayHandle<Scalar4> h_position(
  ...
  // zero forces so we don't leave any forces set for indices that are no longer
  // part of our group
  ...
  for (unsigned int i = 0; i < m_group->getNumMembers(); i++) {
    ...
    if (translational_force_constant == 0) {
      ...
      vec3<Scalar> dr =
          vec3<Scalar>(box.minImage(vec_to_scalar3(position - r0)));
...
void export_HarmonicForceCompute(py::module& m) {
  py::class_<HarmonicForceCompute, std::shared_ptr<HarmonicForceCompute> >(
      m, "HarmonicForceCompute", py::base<ForceCompute>())
      .def(py::init<std::shared_ptr<SystemDefinition>,
                    std::shared_ptr<ParticleGroup>, py::list, py::list, Scalar,
                    Scalar>());
}
